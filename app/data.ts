...
const lipstickCustom = (priceWithTax: number) => {
  switch (true) {
    // https://www.datvietcargo.com/dichvu
    case priceWithTax >= 50:
      return Math.ceil(priceWithTax * 5) / 100;
    case priceWithTax >= 30:
      return 2;
    case priceWithTax >= 10:
      return 1;
    default:
      return 0.5;
  }
...
const perfumeCustom = (priceWithTax: number) => {
  const custom = priceWithTax >= 100 ? Math.ceil(priceWithTax * 7) / 100 : 5; // https://www.datvietcargo.com/dichvu
...
export const price = derived(
  ...
  ([originalPrice, discount]) =>
    (Number(originalPrice) * (100 - Number(discount))) / 100,
);
...
const shippingWeight = derived(
  [category, volume, size, weight],
  ([category, volume, size, weight]) => {
    switch (category) {
      case "perfume":
...
const shipping = derived(
  shippingWeight,
  (shippingWeight) => Math.ceil(shippingCost * shippingWeight),
);

const custom = derived(
  [category, priceWithTax, shippingWeight],
  ([category, priceWithTax, shippingWeight]) => {
    switch (category) {
      case "perfume":
...
export const finalUsd = derived(
  [priceWithTax, shipping, custom],
  ([priceWithTax, shipping, custom]) => priceWithTax + shipping + custom,
);
export const finalVnd = derived(
  [finalUsd, usdToVnd],
  ([finalUsd, usdToVnd]) => finalUsd * Number(usdToVnd),
);
export const profitVnd = derived(
  finalVnd,
  (finalVnd) => Math.min(Math.max(200_000, Math.ceil(finalVnd * 0.1)), 250_000),
);
export const finalWithProfitVnd = derived(
  [finalVnd, profitVnd],
  ([finalVnd, profitVnd]) =>
    Math.ceil((finalVnd + Number(profitVnd)) / 50_000) * 50_000,
);
